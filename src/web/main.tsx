...
function App() {
  ...
  const start = async (scenario: string, protectedFlow = true) => {
    ...
    try {
      const next = await api<Task>('/api/tasks', { method: 'POST', headers: { 'content-type': 'application/json', 'idempotency-key': crypto.randomUUID() }, body: JSON.stringify({ claim, scenario, execution_mode: 'live', seller_count: auditMode ? 2 : 4, protected: protectedFlow, decompose: auditMode ? false : decompose, task_type: auditMode ? 'document_audit' : 'claim_verification', payment_amount_sol: 0.05 }) });
  ...
  return <div className="app-shell">
    <aside className="app-sidebar">
      ...
      <div className="devnet-note"><span>▱</span><p><strong>{config?.settlement === 'DEVNET' ? 'Settling on devnet' : 'Settlement simulated'}</strong><small>{config?.settlement === 'DEVNET' ? 'Each slot settles in its own transaction.' : 'No blockchain transaction is sent.'}</small></p><div className="mini-mountain"/></div>
    </aside>
    <main className="app-main">
      <header className="topbar"><div><span>Workspace</span><b>/</b><strong>{pageLabel}</strong></div><div><span className="worker-status"><i/>{config ? 'LIVE · Worker connected' : 'Connecting…'}</span><span className="network-chip">{config?.settlement === 'DEVNET' ? 'Solana devnet' : 'Simulated settlement'}</span><span className="user-avatar">C</span></div></header>
